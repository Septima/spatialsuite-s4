    		...
    		if (query.type == "list.force" || totalThemeCount<query.limit){
    			for (var i=0;i<groupsWithMatchingThemes.length;i++){
    				...
    				for (var j=0;j<themes.length;j++){
    					...
    					if (query.hasTarget){
        					...
        					var result1 = queryResult.addResult(theme.theme.displayname + " (" + this.themePhrase + ")", theme.description, " ", theme);
                            result1.image = theme.theme.img;
    					}
    				}
    			}
    			...
    			for (var i=0;i<groupsWithMatchingThemes.length;i++){
    				...
    				if (group.themes.length==1 || group.themes.length<freeSlots){
    					...
    					var result3 = queryResult.addNewQuery(group.group.group.displayname + " (" + count + " " + this.themesPhrase + ")", groupDescription, group.group.group.displayname + ": " + query.queryString, null, null, null)
